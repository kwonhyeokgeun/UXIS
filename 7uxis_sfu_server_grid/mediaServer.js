...
io.on('connection', function(socket) {
    ...
    socket.on("disconnect", () => {
        try {
            ...
            if(users[roomId] ==undefined){
                ...
                */
                delete roomToTime[roomId];  //roomToTime 할당해제
    ...
    socket.on('message', function(data) {
        ...
        if(data.message !== ""){
            ...
            /*mysql에 대화 정보 저장*/
            var sql = "INSERT INTO chat_db2 (userName, role, roomId, chat_img, roomTime, msg) VALUES ('"+ data.userName +"','"+data.role+"','"+ data.roomId+"','"+data.chat_img+"','"+data.roomTime+"','"+data.message+"')";
		    connection.query(sql, function (err, result) {
			if (err) throw err;
    ...
    socket.on('set_roomTime', function(data) {
        //mysql에 사용할 room의 고유시간 설정
        roomToTime[data.roomId]=data.time
    })  

    socket.on('req_roomTime', function(data) {
        //mysql에 사용할 room의 고유시간 설정
        var time = roomToTime[data.roomId]
        socket.emit('get_roomTime',{time : time})
    })  
});
...
function createReceiverPeerConnection(socketId, socket, roomId) {
    ...
    pc.ontrack = (e) => {
        ...
        connection.query("SELECT * FROM chat_db2 where roomId = '"+roomId+"' and roomTime ="+roomToTime[roomId], function (err, result, fields) {
            if (err) throw err;
            //console.log(result);
            console.log('request_chat');
            socket.emit('get_chat',result)
